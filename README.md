...
2. Restart AI Client (Claude, Cursor, Codex, etc) and Roblox Studio
3. Done!

# Roblox Studio MCP Server

This repository contains a reference implementation of the Model Context Protocol (MCP) that enables
communication between Roblox Studio via a plugin and clients like [Claude Desktop](https://claude.ai/download), [Cursor](https://www.cursor.com/), or Codex.
...
- A `rmcp` server that talks to MCP clients via `stdio` transport.

When an LLM requests to run a tool, the plugin will get a request through the long polling and post a
response. It will cause responses to be sent to the connected client.
...
The setup process also contains a small plugin installation and MCP client configuration scripts.
...
This MCP Server supports pretty much any MCP Client but will automatically set up [Claude Desktop](https://claude.ai/download), [Cursor](https://www.cursor.com/), and Codex if found.

To set up automatically:

1. Ensure you have [Roblox Studio](https://create.roblox.com/docs/en-us/studio/setup),
   and [Claude Desktop](https://claude.ai/download)/[Cursor](https://www.cursor.com/) installed and started at least once. If you use Codex, run it at least once so `~/.codex/config.toml` exists.
...
1. Download and run the installer:
   ...
   1. Restart Claude/Cursor/Codex and Roblox Studio if they are running.
...
Codex command would look like this:
```sh
codex mcp add Roblox_Studio -- '/Applications/RobloxStudioMCP.app/Contents/MacOS/rbx-studio-mcp' --stdio
```
...
1. Run the following command from the root of this repository.
   ...
   This command carries out the following actions:
      ...
      - Sets up supported MCP clients (Claude, Cursor, Codex) to communicate with the MCP server when their config directories are present.
